...
if [ $? -eq 0 ]; then
    ...
    if [ -f ".env" ] && grep -q "DEEPSEEK_API_KEY=sk-" .env && grep -q "OKX_API_KEY=" .env && ! grep -q "your-.*-here" .env; then
        ...
        echo ""
        echo "📋 启动服务:"
        echo "   启动Web界面: ./start_services.sh"
        echo "   启动交易Bot: ./run.sh"
    else
    echo "📋 下一步操作:"
    echo "   1. 配置 API 密钥: nano .env"
    echo "   2. 启动Web界面: ./start_services.sh"
    echo "   3. (可选)启动交易Bot: ./run.sh"
    fi
    ...
    echo "   后端: http://${SERVER_IP}:5001"
    echo ""
    echo "📚 更多信息请查看:"
    echo "   快速开始: STARTUP_GUIDE.md"
    echo "   部署文档: docs/DEPLOYMENT.md"
    echo ""
    echo "🔧 管理命令:"
    echo "   - 启动Web服务: ./start_services.sh"
    echo "   - 启动交易Bot: ./run.sh"
