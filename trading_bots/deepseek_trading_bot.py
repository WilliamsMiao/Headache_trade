...
def setup_exchange():
    ...
    try:
        ...
        if usdt_balance == 0:
            print("⚠️ 账户余额为0，将运行演示模式（不执行实际交易）")
            TRADE_CONFIG['demo_mode'] = True
...
def export_dashboard_data(price_data, signal_data=None):
    ...
    try:
        ...
        # 如果是演示模式，使用模拟余额
        if TRADE_CONFIG.get('demo_mode', False):
            usdt_total = 10000.0  # 演示模式使用10000 USDT
            usdt_free = 10000.0
...
def get_sentiment_indicators():
    ...
    try:
        ...
        API_KEY = os.getenv('CRYPTORACLE_API_KEY', '')  # 从环境变量读取
...
def execute_intelligent_trade(signal_data, price_data):
    ...
    print("\n" + "="*60)
    print("🔥 开始执行交易流程...")
    print(f"📊 信号详情: {signal_data['signal']} | 信心: {signal_data['confidence']}")
    ...
    try:
        ...
        sys.stdout.flush()
        current_position = get_current_position()
        print(f"✅ 持仓获取成功: {current_position}")
        sys.stdout.flush()
    except Exception as e:
        print(f"❌ 获取持仓失败: {e}")
        import traceback
        traceback.print_exc()
        print("⚠️ 继续执行价格分析和信号生成，跳过交易执行")
        sys.stdout.flush()
        return
    ...
    try:
        ...
        if "51010" in error_msg or "account mode" in error_msg.lower():
            print("⚠️ OKX账户模式错误 - 请检查OKX账户设置:")
            print("   1. 确保开通了合约交易权限")
            print("   2. 账户设置为'统一账户'模式")
            print("   3. 检查API权限是否包含交易权限")
            print("🔄 继续执行价格分析和信号生成，跳过交易执行")
            return
        
        # 如果是持仓不存在的错误，尝试直接开新仓
        if "don't have any positions" in error_msg:
            print("⚠️ 尝试直接开新仓...")
            try:
                if signal_data['signal'] == 'BUY':
                    exchange.create_market_order(
                ...
                print(f"❌ 直接开仓也失败: {e2}")
                print("🔄 继续执行后续分析，跳过本次交易")
        
        # 打印详细错误但不终止程序
        import traceback
        print("📋 详细错误信息:")
        traceback.print_exc()
        print("🔄 程序继续运行，等待下一个交易周期...")
